...
#video_input = "videos/japan_factory.mp4"
out_folder = 'output'
output = out_folder +"/output_video.mp4" 
output_json = out_folder +"/output_data.json"
output_groups = out_folder+'/groups'
skip_frame = 30 #object detection afer every ith frame
confidence_level = 0.4
group_size = 3

#used to calculate ellipse: a=allowed_distance b=allowed_distance*shrink_factor
allowed_distance = 150 
shrink_factor = 0.7

#in how many frames people must stand together to call it a group
group_duration = 50
################# END parameters #################
...
def write_json(data, filename=output_json): 
    ...
    while dep_keys != []:
        ...
        if not group_id in global_group_dict.keys():
            ...
            data = {'group_nr' : group_count,
                    ...
                    'duration' : 1,
                    'persons': group,
                    'detected':False} #depending on group_duration
...
write_json(global_group_dict,filename=out_folder +'/overview.json')
