# Author: https://github.com/IBM/dnn-object-detection
...
while (vid.isOpened()):
    ...
    if not ret:
        ...
        retval, thresholdImage = cv2.threshold(differenceFrame,
        ...
        blobs = list(
            filter(lambda c: cv2.contourArea(c) > BLOB_SIZE, contours))
        if blobs:
            # print(blobs[0])
            ...
            cv2.matchTemplate(last_frame_captured, frame, eval(methods[1]))[0]
            ...
            for c in blobs:
                ...
                if tracked_blobs:
                    ...
                    closest_blobs = sorted(tracked_blobs,
                                           key=lambda b: cv2.norm(
                                               b['trail'][0], center))
                    # print( tracked_blobs)
                    ...
                    for close_blob in closest_blobs:
                        ...
                        if distance < BLOB_LOCKON_DISTANCE_PX:
                            ...
                            if expected_dir == 'left' and \
                                    close_blob['trail'][0][0] < center[0]:
                                continue
                            elif expected_dir == 'right' and \
                                    close_blob['trail'][0][0] > center[0]:
                                continue
                            else:
                ...
                if not closest_blob:
                    ...
                    data = {'filename': filename, 'time': timestamp,
                            'channel': cam_channel, 'location': '47'}
                    # print(data)
                    print("posting " + filename + " to node")
                    r = requests.post(url, files=files, data=data)
                    # print(r.text)
                    time.sleep(
                        2)  # TODO, instead of pausing perhaps we can record video until motion stops?
                    # Timer(5, save_screenshot(frame), ()).start()
                    views1 = np.hstack((gray, cv2.convertScaleAbs(avg)))
                    views2 = np.hstack((differenceFrame, thresholdImage))
                    threshold_images = np.hstack(
                        (thresholdImage, threshold_images))
                    all_views = np.vstack((views1, views2))
        ...
        if tracked_blobs:
            ...
            for i in range(len(tracked_blobs) - 1, -1, -1):
                if frame_time - tracked_blobs[i][
                    'last_seen'] > BLOB_TRACK_TIMEOUT:
